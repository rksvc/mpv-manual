...
:   Adjust `--video-align-x` relatively to the OSD width, rather than
    relatively to the video width like the option. This is useful to pan
...
:   Pan the video while holding a mouse button, keeping the clicked part
    of the video under the cursor.

`align-to-cursor`

:   Pan through the whole video while holding a mouse button, or after
    clicking once if `toggle_align_to_cursor` is `yes`.
