---
title: Context menu script
---

This script provides a context menu for platforms without integration
with a native context menu. On these platforms, it can be disabled
entirely using the `--load-context-menu=no` option. On platforms where
the integration is implemented, it is already disabled by default.
