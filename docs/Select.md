...
:   Select a file from the watch history. Requires
    ...
    If you don't already use `--autocreate-playlist`, it is recommended
    to enable it with this script binding to populate the playlist with
    the other files in the entry's directory.
...
:   Select a file from watch later config files (see [RESUMING
    ...
    Example for input.conf to play files adjacent to the watch later
    entry
