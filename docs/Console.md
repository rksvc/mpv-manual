...
:   Select the first completion if one wasn't already manually selected,
...
:   Move the cursor to the end of the current word, or if between words,
...
:   Hide the console if the current line is empty, otherwise delete the
    next character.

Ctrl+BACKSPACE and Ctrl+w

:   Delete text from the cursor to the beginning of the current word, or
    if between words, to the beginning of the previous word.

Ctrl+DEL and Alt+d

:   Delete text from the cursor to the end of the current word, or if
...
:   Search the command history. See [SELECT](Select.md) for the key
